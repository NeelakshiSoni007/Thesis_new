...
class CertMng():
    ...
    def generate(self):
        ...
        if not os.path.exists(self.certFile) and not os.path.exists(self.keyFile):
...
def isDocker():
    ...
    The process's PID inside the container differs from its PID on the host
...
def main():
    ...
    app.run(port=FLASK_PORT, debug=True, host=HOST)
